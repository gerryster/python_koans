...
class AboutGenerators(Koan):
    ...
    def test_generator_method_will_yield_values_during_iteration(self):
        ...
        for item in self.simple_generator_method():
            result.append(item)
        self.assertEqual('peanut butter and jelly'.split(), result)
    ...
    def test_generators_can_act_as_coroutines(self):
        ...
        next(generator)
        # gerryster: if this was not done then the coroutine would be suspended
        # half way through its execution.
    ...
    def test_before_sending_a_value_to_a_generator_next_must_be_called(self):
        ...
        try:
            ...
            self.assertRegex(ex.args[0], 'can\'t send.*to a just-started generator')
